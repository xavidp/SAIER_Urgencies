...
my_doc <- read_docx(path=file.path(getwd(), docpath, "2017_MODEL en blanc_B_v02.docx") )
# styles_info(my_doc)

my_doc <- my_doc %>% 
  body_add_par(value = "Derivacions del CUESB al SAIER (2017)", style = "Balloon Text") %>% 
  body_add_par(value = "Table of content", style = "table title") %>% 
...
my_doc <- my_doc %>% 
  ...
  body_add_img(src = file.path(getwd(), imgpath, "2017.worldmap.familysize.png"), width = 8, height = 4, style = "centered") %>% 
  body_add_par("", style = "Normal") # blank paragraph
