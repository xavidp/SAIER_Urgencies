...
#  geom_text(aes(label=n), vjust=1.6, color="white", size=6.5) +
  geom_text(aes(label=n), vjust=-0.2, color="blue", size=6.5) +
...
ggplot.2017.pais.tramit.ue <- ggplot(data=my_data7g, aes(x=PaisTramitSituacioAdministrativa, y=n)) +
  geom_bar(stat="identity") +
  #scale_x_discrete(breaks = NULL) +
  geom_text(aes(label=n), vjust=1.3, color="white", size=6.5) +
  #geom_text(aes(label=n), vjust=1.6, color="white", size=3.5) +
  labs(x="Denegada la Protecció Internacional a un altre País de la UEB", y="N") + 
  #scale_fill_discrete(name="Continents") + # Legend Title
  theme(axis.text.x = element_text(angle = 0, hjust = 0.5, vjust=0.5, size=14),
        axis.text.y = element_text(size=14),
        axis.title.x = element_text(size=14),
...
ggplot.2017.perfils <- ggplot(data=my_data7h, aes(x=Perfil, y=n)) +
  ...
  #scale_x_discrete(breaks = NULL) +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 10)) +
  geom_text(aes(label=n), vjust=-0.1, color="blue", size=5.5) +
  labs(x="Perfils", y="N") + 
...
my_doc <- my_doc %>% 
  ...
  body_add_par(value = "Perfils", style = "heading 1") %>% 
  body_add_par("", style = "Normal") %>% # blank paragraph
  body_add_par(value = "Gràfic de barres", style = "heading 2") %>% 
  body_add_par("", style = "Normal") %>% # blank paragraph
...
my_doc <- my_doc %>% 
  body_add_break() %>% 
  body_add_par(value = "Decretada o recent majoria d'edat (Refugiat o Immigrant)", style = "heading 2") %>% 
  body_add_par("", style = "Normal") %>% # blank paragraph
...
my_doc <- my_doc %>% 
  ...
  body_add_par("", style = "Normal") %>% # blank paragraph
  body_add_par("Evolució mensual de les derivacions d'usuaris del SAIER al CUESB", style = "Balloon Text")
...
colnames(my_data7k.t) <- c("Casos", "N")
