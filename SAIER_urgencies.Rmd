...
checkpoint("2018-03-01") # Keep R packages from this date for reproducibility
...
# Derived from: https://blog.learningtree.com/how-to-display-data-on-a-world-map-in-r/
  ...
  if (!require("ggplot2")) install.packages("ggplot2");  devtools::install_github('hadley/ggplot2')
...
my_doc <- read_docx(path=file.path(getwd(), templatepath, "2017.Word_AAI_template_v02.docx") )
#my_doc <- read_docx()
# styles_info(my_doc)
...
my_doc <- my_doc %>% 
  ...
  body_add_par(value = "Table of contents", style = "Balloon Text") %>% 
...
my_doc <- my_doc %>% 
  body_add_break() %>% 
  body_add_par(value = "Demo de Taula de dades d'Entrada. 2017", style = "heading 2") %>% 
  body_add_par("", style = "Normal") %>% # blank paragraph
  body_add_table(head(my_data5b), style = "table_template") %>%
  body_add_par(value = "Demo de Taula de dades d'Entrada. 2017", style = "table title")
...
# 
# #plotly.2017.nacionalitat <- my_data %>% count(NacionalitatAngles, ContinentNacio) %>%
# #  plot_ly(x = ~NacionalitatAngles, y = ~n, color = ~ContinentNacio)
...
ggplot.2017.nacionalitat <- ggplot(data=my_data7a, aes(x=NacionalitatAngles, color=ContinentNacio, fill=ContinentNacio, y=n)) +
...
my_doc <- my_doc %>% 
  body_add_break() %>% 
  body_add_par(value = "Nacionalitats per continents", style = "heading 2") %>% 
  body_add_par("", style = "Normal") %>% # blank paragraph
  body_add_par(value = "Totals i per països", style = "heading 3") %>% 
  body_add_par("", style = "Normal") %>% # blank paragraph
  body_add_img(src = file.path(getwd(), imgpath, "2017.barchart.nacionality.png"), width = 6, height = 3, style = "centered") %>% 
  body_add_par("", style = "Normal") # blank paragraph


# Convertir a versió plotly del gràfic de ggplot
ggplotly(ggplotly.2017.nacionalitat = ggplot.2017.nacionalitat)
...
# # Faceting also --------------------------------------
# ggplot.2017.nacionalitat.faceted <- ggplot(data=my_data7a, aes(x=NacionalitatAngles, color=ContinentNacio, fill=ContinentNacio, y=n)) +
#   geom_bar(stat="identity") +
#   geom_text(aes(label=n), vjust=1.6, color="white", size=2.5) +
#   theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.5)) +
#   facet_wrap(~ContinentNacio)
# 
...
# # Afegir gràfic al word
# my_doc <- my_doc %>% 
#   body_add_par(value = "Faceted bar chart", style = "heading 3") %>% 
#   body_add_par("", style = "Normal") %>% # blank paragraph
...
## Gender of users. World map with pie charts integrated overtime
...
my_doc <- my_doc %>% 
  body_add_break() %>% 
  body_add_par(value = "Gènere d'Usuaris derivats del SAIER al CUESB. 2017", style = "heading 2") %>% 
  body_add_par("", style = "Normal") %>% # blank paragraph
  body_add_img(src = file.path(getwd(), imgpath, "2017.leaflet.gender.png"), width = 6, height = 3, style = "centered") %>% 
  body_add_par("", style = "Normal") %>% # blank paragraph
  body_add_par("Blau: Homes; Rosa: Dones. Groc: Desconegut", style = "Normal")
...
my_doc <- my_doc %>% 
  body_add_break() %>% 
  body_add_par(value = "Nuclis familiars", style = "heading 2") %>% 
  body_add_par("", style = "Normal") %>% # blank paragraph
  body_add_par(value = "D'on són les famílies més nombroses o els individus sols?", style = "heading 3") %>% 
...
# Afegir contingut a l'informe en Word
my_doc <- my_doc %>% 
  body_add_par(value = "Distribució d'adults i menors per països", style = "heading 3") %>% 
  body_add_par("", style = "Normal") %>% # blank paragraph
  body_add_img(src = file.path(getwd(), imgpath, "2017.leaflet.agegroups.png"), width = 6, height = 3, style = "centered") %>% 
  body_add_par("", style = "Normal") %>% # blank paragraph
  body_add_par("Verd fosc: Adult | Verd clar: Menor | Taronja: Desconegut", style = "Normal")
...
# #Codi extra per a extreure taula de dades de resultats, fent "web craping"" amb rvest
...
my_doc <- my_doc %>% 
  body_add_break() %>% 
  body_add_par(value = "Situació Administrativa", style = "heading 2") %>% 
  body_add_par("", style = "Normal") %>% # blank paragraph
  body_add_par(value = "Evolució mensual i Totals", style = "heading 3") %>% 
  body_add_par("", style = "Normal") %>% # blank paragraph
  body_add_img(src = file.path(getwd(), imgpath, "2017.pt.SituacioAdministrativa.t.png"), width = 6, height = 4, style = "centered") %>% 
  body_add_par("", style = "Normal") %>% # blank paragraph
  body_add_par("La intensitat del color vermell és proporcional a la suma de casos de cada tipus", style = "Normal")
...
# Afegir contingut a l'informe en Word
my_doc <- my_doc %>% 
  body_add_par(value = "Gràfic de barres de Situacions Administratives", style = "heading 3") %>% 
  body_add_par("", style = "Normal") %>% # blank paragraph
...
## Afegir gràfic al word
#my_doc <- body_add_chart(my_doc, chart = my_barchart, style = "centered")

#   
#   # Afegir contingut a l'informe en Word
# my_doc <- my_doc %>% 
#   body_add_par(value = "Denegats a un altre País Europeu", style = "heading 3") %>% 
#   body_add_img(src = file.path(getwd(), imgpath, "2017.pt.SituacioAdministrativa.g.png"), width = 6, height = 4, style = "centered") %>% 
#   body_add_par("", style = "Normal") %>% # blank paragraph
...
my_doc <- my_doc %>% 
  body_add_break() %>% 
  body_add_par(value = "Perfils", style = "heading 2") %>% 
  body_add_par("", style = "Normal") %>% # blank paragraph
  body_add_par(value = "Casos segons relació entre Situació Administrativa i Perfil", style = "heading 3") %>% 
  body_add_par("", style = "Normal") %>% # blank paragraph
  body_add_img(src = file.path(getwd(), imgpath, "2017.sankey.sa.p.png"), width = 6, height = 4, style = "centered") %>% 
  body_add_par("", style = "Normal") %>% # blank paragraph
  body_add_par("NO CONSTA: No es disposa de dades", style = "Normal")
...
print(my_doc, target=file.path(getwd(), docpath, "2017.Derivacions_CUESB_a_SAIER.docx"))
invisible()
