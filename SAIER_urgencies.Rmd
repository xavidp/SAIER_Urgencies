...
#my_doc <- read_docx(path=file.path(getwd(), templatepath, "2017.Word_AAI_template.docx") )
my_doc <- read_docx()
# styles_info(my_doc)

my_doc <- my_doc %>% 
  body_add_par(value = "Derivacions del CUESB al SAIER (2017)", style = "heading 1") %>% 
...
#  body_add_img(src = file.path(getwd(), imgpath, "2017.leaflet.gender.png"), width = 6, height = 3, style = "centered") %>% 
  body_add_par("", style = "Normal") %>% # blank paragraph
  body_add_par("Pendent de fer", style = "graphic title") %>%
  body_add_par("Pendent de fer", style = "Normal")
...
png(filename=file.path(getwd(), imgpath, "2017.barchart.nacionality.png"), width=1200, height=600, units="px")
...
ggplotly(ggplotly.2017.nacionalitat = ggplot2::last_plot(),
         tooltip = "all",
         dynamicTicks = FALSE)
